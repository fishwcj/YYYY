...
public class JZ_Activity extends FragmentActivity implements RecognitionListener {
...
//		});
		...
		kind.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				startActivity(new Intent(JZ_Activity.this, SelectPicPopupWindow.class));
			}
		});
		...
		button_ok.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (consumString.length() > 0) {
					...
					consume.setText(consumString.toCharArray(), 0, consumString.length());
					...
					if (inOrOut == 0) {
						...
						// 更新显示余额
						Float remain = Float.parseFloat(budgetRemain.getText().toString());
				...
				backgroundColor.refreshback();
				// 更新消费
				String consumed = new DecimalFormat("0.0").format(Index_Activity.budget
						- Index_Activity.remain);
				JZ_Activity.consumed.setText(consumed);
				inputCheck.setViewString("");
				Toast.makeText(JZ_Activity.this, "成功记入一笔!", Toast.LENGTH_LONG).show();
			}
		});
		/*
		 * 按钮设置预算的按钮
	...
	public void onResults(Bundle results) {
		...
		String[] iStrings = nbest.toArray(new String[nbest.size()]);

		for (int i = 0; i < iStrings.length; i++)
			System.out.println("第" + i + "个字符串:" + iStrings[i]);
		System.out.println("识别成功：" + Arrays.toString(nbest.toArray(new String[nbest.size()])));

		VoiceString voiceString = new VoiceString();
		String[] result = voiceString.dealWithOrigin_Result(iStrings);

		kind.setText(result[0]);
		System.out.println("返回的钱字符串：" + result[1]);
		consume.setText(result[1]);
